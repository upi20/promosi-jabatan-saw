...
class HakAksesLevel extends Render_Controller
{
    ...
    {
        $usr = $this->jabatan->getDataDetail($id);
        if ($usr) {
            // Page Settings
            $this->title                     = 'Pengaturan Hak Akses Jabatan ' . $usr['lev_nama'];
            $this->content                     = 'pengaturan-level-hakakses';
            $this->navigation                 = ['Pengaturan', 'Jabatan', 'Hak Akses'];
            $this->plugins                     = ['datatables'];

            // Breadcrumb setting
            $this->breadcrumb_1             = 'Pengaturan';
            $this->breadcrumb_1_url         = '#';
            $this->breadcrumb_2             = 'Jabatan';
            $this->breadcrumb_2_url         = base_url() . 'pengaturan/jabatan';
