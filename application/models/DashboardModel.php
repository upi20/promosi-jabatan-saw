...
class DashboardModel extends Render_Model
{
	...
	{
		return $this->db->get_where("role_users", ['role_lev_id' => '6'])->num_rows();
	}
	public function jmlManajer()
	{
		return $this->db->get_where("role_users", ['role_lev_id' => '7'])->num_rows();
	}

	public function jmlAdmin()
	{
		return $this->db->get_where("role_users", ['role_lev_id' => '2'])->num_rows();
	}

	public function jmlKaryawanBelumDiIsi()
	{
		return $this->db->select("*")
			->from("role_users a")
			->join("nilai b", "a.role_user_id = b.id_user")
			->where("a.role_lev_id", "6")
			->get()
			->num_rows();
	}
